          ...
          >
            <div class="option">
              <div class="search-box-item">
                ...
                <span style="color: #c73a64; font-size: 13px"
      ...
      <div class="main-container">
        <div class="left-section">
          ...
          <div class="rate-grid" v-if="!isPrintMode">
            ...
            <el-input v-model="rateEUR" placeholder="USD" type="number"
...
// 货币相关状态
const selectedCurrency = ref("USD"); // 默认使用的美元
const exchangeRate = ref(1); // 汇率储存，默认为 人民币兑人民币，为1
const rateEUR = ref(1.1634); // 欧元汇率
...
// const Delivery = ref("不含税交货价"); // 交货相关信息
const Delivery = ref(""); // 交货相关信息
// 新增计算属性
...
const customerTypesNAV = [
  ...
  { type: "Wholesaler", name: "批发商(不含税RRP0.62)" },
  { type: "Distributor", name: "分销商(不含税RRP0.65)" },
  { type: "OEM", name: "OEM(不含税RRP0.67)" },
  { type: "Dealer", name: "零售(不含税RRP*0.7)" },
  { type: "Retail", name: "普通(不含税RRP*0.72)" },
];
...
const getCurrencySymbol = () => {
  ...
  return symbols[selectedCurrency.value]; // 默认为美元符号
};
// 处理tableData中的货币变更
const handleCurrencyChange = (newCurrency) => {
  // 根据选中的货币设置汇率
  if (newCurrency === "USD") {
    exchangeRate.value = 1; // 美元汇率固定为1
...
const dataProcessMethods = () => {
  tableData.value = originalTableData.value; //同步数据
...
// 新增语言切换相关逻辑
const currentLanguage = ref("en"); // 默认为英文
...
// 新增银行切换相关逻辑
const currentbank = ref("hongkong"); // 默认为香港收款
const togglebank = () => {
