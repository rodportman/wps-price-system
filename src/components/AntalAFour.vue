<template>
  <div class="container">
    ...
    <div class="header">
      ...
      <h2 v-if="currentLanguage === 'zh'" style="margin-bottom: 0px">
        深圳市威普森船舶科技有限公司
      </h2>
      <h2 v-else style="margin-left: 10px">WHEELPERSON MARINE CO.,LTD</h2>
      <p v-if="currentLanguage === 'zh'">船艇设备一站式服务商</p>
      <p v-else style="margin-left: 10px">
        Marine Equipment One-Stop Service Provider
      </p>
      <p v-if="!isPrintMode">（Antal A4版）</p>
    ...
    <div class="table-container">
      <template v-for="(page, pageIndex) in pagedTableData" :key="pageIndex">
        <el-table
          :data="page.data"
          style="width: 100%"
          :fit="true"
        ...
        >
          ...
          <el-table-column
            :label="
              currentLanguage === 'zh' ? '产品描述' : 'Product Description'
            "
            min-width="180"
          >
            <template #default="scope">
        ...
        <div
          v-if="pageIndex !== pagedTableData.length - 1"
          class="page-break"
        ></div>
      </template>
      <!-- 总计 -->
      <div class="total">
        ...
        ><span v-else> Total Amount: </span>{{ formatDisplayCurrency(total) }}
      </div>
    </div>
    ...
    <div class="footer">
      <div class="footer-text">
        ...
        >
          ...
          <p>
            6.地址：深圳市大鹏新区葵涌街道坝光社区丰树山路2号坝光新村西区8栋8B2401
          </p>
...
// 手动分页
const PAGE_BREAKS = [9, 21, 33, 45, 57, 69, 81]; // 分页点
const pagedTableData = computed(() => {
  const pages = [];
  let start = 0;
  for (let i = 0; i < PAGE_BREAKS.length; i++) {
    const end = Math.min(PAGE_BREAKS[i], tableData.value.length);
    if (start < end) {
      pages.push({
        data: tableData.value.slice(start, end),
        startIndex: start,
      });
      start = end;
    }
  }
  if (start < tableData.value.length) {
    pages.push({
      data: tableData.value.slice(start, tableData.value.length),
      startIndex: start,
    });
  }
  return pages;
...
const generatePDF = async () => {
  try {
    ...
    ElMessage.success("报价成功！请查看下载的文件");
  } catch (error) {
...
// 拖拽功能的相关实现！！！！
// 分页功能时，tbody未实体化，的额外解决方案
const sortableInstances = new Map();
const initSort = () => {
  // await nextTick();
  const tbodies = Array.from(
    document.querySelectorAll(".sortable-table .el-table__body-wrapper tbody")
  );
  console.log("测");
  tbodies.forEach((tbody) => {
    console.log("测试");
    // 如果已有实例，先销毁（防止重复绑定）
    const existing = sortableInstances.get(tbody);
    if (existing && existing.destroy) {
      try {
        existing.destroy();
        console.log("测试1");
      } catch (e) {
        console.warn("销毁 Sortable 实例失败：", e);
      }
      sortableInstances.delete(tbody);
      console.log("测试2");
    }
    console.log("测试3");
    // 创建新实例
      ...
      onEnd: (evt) => {
        try {
          ...
          if (oldIndex == null || newIndex == null || oldIndex === newIndex)
            return;
          // 重新处理显示数据、金额等（会刷新 tableData 与 pagedTableData，使序号自动更新）
          // dataProcessMethods();
          // 重新初始化以防 DOM 变化导致实例失效
          // nextTick(() => initSort());
        } catch (e) {
          console.error("onEnd 处理失败：", e);
        }
      },
    });
    sortableInstances.set(tbody, inst);
  });
};
watch(
  pagedTableData,
